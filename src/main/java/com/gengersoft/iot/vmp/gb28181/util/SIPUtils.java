...
import com.gengersoft.iot.vmp.gb28181.bean.RemoteAddressInfo;
...
public class SIPUtils {
    ...
    public static UserAgentHeader createUserAgentHeader(SipFactory sipFactory, GitUtils gitUtils) throws PeerUnavailableException, ParseException {
        List<String> agentParam = new ArrayList<>();
        agentParam.add("wvp-GB28181-improve");
        if (gitUtils != null ) {
            if (!ObjectUtils.isEmpty(gitUtils.getBuildVersion())) {
                agentParam.add("v");
                agentParam.add(gitUtils.getBuildVersion() + ".");
            }
            if (!ObjectUtils.isEmpty(gitUtils.getCommitTime())) {
                agentParam.add(gitUtils.getCommitTime());
            }
        }
